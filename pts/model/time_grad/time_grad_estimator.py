...
class TimeGradEstimator(PyTorchEstimator):
    def __init__(
        ...
        num_layers: int = 2,  # Теперь это количество слоев TST
        hidden_size: int = 64,  # Заменили num_cells на hidden_size
        ...
        use_pretrained: bool = True,  # Новый параметр для использования предобученной модели
        **kwargs,
    ) -> None:
        ...
        self.hidden_size = hidden_size  # Размер скрытого состояния TST
        self.num_parallel_samples = num_parallel_samples
        self.dropout_rate = dropout_rate
        self.cardinality = cardinality
        self.embedding_dimension = embedding_dimension
        self.use_pretrained = use_pretrained  # Сохраняем параметр
    ...
    def create_training_network(self, device: torch.device) -> TimeGradTrainingNetwork:
        return TimeGradTrainingNetwork(
            ...
            num_layers=self.num_layers,  # Количество слоев TST
            num_cells=self.hidden_size,  # Размер скрытого состояния
            ...
            use_pretrained=self.use_pretrained,  # Передаем параметр
        ).to(device)
